...
class workflowitems(models.Model):
    ...
    current_state = models.CharField(
        choices=STATE_TYPE, default='DRAFT', max_length=15)
    next_available_transitions = models.CharField(max_length=15)
    current_from_party = models.ForeignKey(
        Parties, on_delete=models.DO_NOTHING, related_name='from_party')
    current_to_party = models.ForeignKey(
        Parties, on_delete=models.DO_NOTHING, related_name='to_party')
    # last_wf_event = models.ForeignKey(workevents,on_delete=models.CASCADE)

    def __str__(self):
        return "%s - %s" % (self.program, self.id)
