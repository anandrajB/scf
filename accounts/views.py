...
from accounts.permission.base_permission import Is_BankAdministrator, Is_PartyAdministrator 
        ...
        if phone and email:
            ...
            if user:
                if user.is_active == False:
                    return Response({"status": "failure", "data": "This user is_inactive , please contact customer support team finflosupport@xyz.com"})
                else:
                    login(request, user)
            ...
            return Response(
                {"status": "failure", "data": "Unable to login with given credentials"} , status=status.HTTP_400_BAD_REQUEST
            )
        return Response(
            {
                "status": "failure",
    ...
    def post(self, request):
        ...
        if phone and email:
            ...
            if user:
                ...
                return Response({"status": "failure", "data": "given details doesn't exists"}, status=status.HTTP_404_NOT_FOUND)
        return Response(
            {
                "status": "failure",
                "data": "You need to provide both phone and email",
    ...
    def post(self, request):
        ...
        if phone and email:
            ...
            if user and str(cc.otp) == OTP:
                ...
                return Response({"status": "failure", "data": "Wrong OTP "},status=status.HTTP_406_NOT_ACCEPTABLE)
        return Response(
            {
                "status": "failure",
...
class Inactiveuser(ListAPIView):
    ...
    permission_classes = [Is_BankAdministrator]
...
class ActionUpdateDeleteApiview(RetrieveUpdateDestroyAPIView):
    queryset = Action.objects.all()
    serializer_class = Actionserializer
    permission_classes = [IsAuthenticated,Is_BankAdministrator , Is_PartyAdministrator]
