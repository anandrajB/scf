...
class workflowitems(models.Model):
    ...
    final_state = models.CharField(choices=STATE_TYPE,default=STATUS_DRAFT,max_length=50)
    next_available_transitions = models.CharField(max_length=255)
    event_users = models.ForeignKey("accounts.customer",on_delete=models.CASCADE,related_name='customername')
...
# --------------------------------------------------------------------------------------------------------
    ...
    def submit(self):
        ...
        self.action = 'SUBMIT'
        ws = workflowitems.objects.get(id=self.id)
        workevents.objects.create(workitems=ws, from_state='DRAFT', to_state = 'AWAITING_APPROVAL' , interim_state = 'AWAITING_SIGN_A' ,from_party = self.current_from_party , to_party = self.current_to_party)
