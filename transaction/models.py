...
class workevents(models.Model):
    ...
    workitems = models.ForeignKey(
        'accounts.workflowitems', on_delete=models.CASCADE, related_name='workflowevent')
    event_user = models.ForeignKey(
        'accounts.customer', on_delete=models.CASCADE, related_name='customername')
    # from_state = models.CharField(max_length=15)
    # to_state = models.CharField(max_length=15)
    from_state = FSMField(choices=STATE_TYPE, default='DRAFT')
    ...
    @transition(field=from_state, source="DRAFT", target="SIGN_A")
    def start(self):
        self.interim_state = 'SIGN_A'
        self.to_state = "AW_AP"
        pass

    @transition(field=from_state, source="SIGN_A", target="SIGN_B")
    def A_to_B(self):
        self.interim_state = 'SIGN_B'
        self.to_state = "AW_AP"
        pass

    @transition(field=from_state, source="SIGN_B", target="SIGN_C")
    def B_to_C(self):
        self.interim_state = "SIGN_C"
        self.to_state = "AW_AP"
        pass
