...
class ProgramCreateApiView(ListCreateAPIView):
    ...
    permission_classes = [IsAuthenticated &
                          Ismaker | IsAuthenticated & Is_Administrator]
...
class InvoiceUploadCreateApiView(GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not is_many:
            ...
            if serializer.is_valid():
                serializer.save(from_party=user.party,
                                to_party=user.party, event_user=user)
                return Response({"status": "success"}, status=status.HTTP_201_CREATED)
            return Response({"status": "failure", "data": serializer.errors})
        else:
...
class InboxListApiview(ListAPIView):
    ...
    def get_queryset(self):
        ...
        user = self.request.user
        record_type = self.request.query_params.get("PROGRAM")
