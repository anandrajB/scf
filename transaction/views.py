...
class SubmitTransitionSign_AApiview(CreateAPIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        obj = generics.get_object_or_404(workflowitems, id=pk)
        party = obj.current_from_party
        user = self.request.user
        user2 = obj.event_users.id
        signs = signatures.objects.get(
            party=party, action='SUBMIT', model='PROGRAM')
        qs = userprocessauth.objects.get(
            user=user2, action='SUBMIT', model_id='PROGRAM')
        if (user.is_sign_a | user.is_administrator == True) and (user.party == party):
            sign_type = signs.sign_a
            if sign_type == True:
                flow = WorkFlow(obj)
                flow.submit_level_1()
                qs.sign_a = True
...
class SubmitTransitionSign_BApiview(CreateAPIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        qs = userprocessauth.objects.get(
            user=user2, action='SUBMIT', model_id='PROGRAM')
        if (user.is_sign_b | user.is_administrator == True) and (user.party == party):
            sign_type = signs.sign_b
            if sign_type == True:
                flow = WorkFlow(obj)
                flow.submit_level_2()
                qs.sign_b = True
...
class SubmitTransitionSign_CApiview(CreateAPIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        qs = userprocessauth.objects.get(
            user=user2, action='SUBMIT', model_id='PROGRAM')
        if (user.is_sign_c | user.is_administrator == True) and (user.party == party):
            sign_type = signs.sign_c
            if sign_type == True:
                flow = WorkFlow(obj)
                flow.submit_level_3()
                qs.sign_c = True
...
class RejectSign_AApiview(CreateAPIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        user2 = obj.event_users.id
        signs = signatures.objects.get(
            party=party, action='REJECT', model='PROGRAM')
        qs = userprocessauth.objects.get(
            user=user2, action='REJECT', model_id='PROGRAM')
        if (user.is_sign_a | user.is_administrator == True) and (user.party == party):
            sign_type = signs.sign_a
            if sign_type == True:
                flow = WorkFlow(obj)
