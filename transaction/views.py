...
class ProgramUpdateDeleteApiview(APIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            qs = data.record_datas
            value = ({"state": mydata, "date": "test"})
            qs = value
            # qs.save()
...
class InboxListApiview(ListAPIView):
    ...
    def get_queryset(self):
        user = self.request.user
        program = request.data.get('program',None)
        if program:
            if user.is_administrator  == True:
                queryset = workevents.objects.filter(
                    to_party__name__contains=self.request.user.party.name,final = "YES",type = program).exclude(from_state='DRAFT').order_by('created_date')
            elif user.party.party_type == "CUSTOMER":
                queryset = workevents.objects.filter(
                    to_party__name__contains=self.request.user.party.name,c_final = "YES").exclude(from_state='DRAFT').order_by('created_date')
...
class DraftListApiview(ListAPIView):
    ...
    def get_queryset(self):
        queryset = workevents.objects.filter(
            to_party__name__contains=self.request.user.party.name, from_state='DRAFT')
        return queryset
