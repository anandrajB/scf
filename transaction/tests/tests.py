...
from transaction.models import Programs, workflowitems, workevents
from accounts.models import Currencies, Countries, Parties, User, userprocessauth, Action, Models
import datetime
...
class mytestcase(TestCase):
    ...
    def test(self):
        ...
        user.save()
        ss = Models.objects.create(
            desc="test", api_route='test_user')
        ss.save()
        c = Client()
        ...
        work = workflowitems.objects.create(
            program=program, current_from_party=party, current_to_party=party2, user=user)
        work.save()
        event = workevents.objects.create(
            workitems=work, from_party=party2, to_party=party2, event_user=user)
        event.save()
        print("1")
        ...
        work = workflowitems.objects.create(
            program=program2, current_from_party=party, current_to_party=party2, user=user)
        work.save()
        event = workevents.objects.create(
            workitems=work, from_party=party2, to_party=party2, event_user=user)
        event.save()
        ...
        work = workflowitems.objects.create(
            program=program3, current_from_party=party, current_to_party=party2, user=user)
        work.save()
        event = workevents.objects.create(
            workitems=work, from_party=party2, to_party=party2, event_user=user)
        event.save()
        ...
        work = workflowitems.objects.create(
            program=program5, current_from_party=party, current_to_party=party2, user=user)
        work.save()
        event = workevents.objects.create(
            workitems=work, from_party=party2, to_party=party2, event_user=user)
        event.save()
