...
class Workeventsmessageserializer(serializers.ModelSerializer):
    ...
    program = serializers.PrimaryKeyRelatedField(
        queryset=workflowitems.objects.all(), source='workitems')
...
class Programcreateserializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        event = workevents.objects.create(
            user=event_user, workitems=work, from_party=from_party, to_party=to_party, type="PROGRAM")
        event.save()
        return program
...
class programupdateserilizer(serializers.Serializer):
    ...
    party = serializers.PrimaryKeyRelatedField(
        queryset=Parties.objects.all(), required=False)
    program_type = serializers.ChoiceField(
        choices=program_type, required=False)
    finance_request_type = serializers.ChoiceField(
        choices=finance_request_type, default=None, required=False)
    ...
    interest_type = serializers.ChoiceField(
        choices=interest_type, required=False)
    interest_rate_type = serializers.ChoiceField(
        choices=interest_rate_type, required=False)
    ...
    def update(self, instance, validated_data):
        ...
        instance.program_type = validated_data.get(
            'program_type', instance.program_type)
        instance.finance_request_type = validated_data.get(
            'finance_request_type', instance.finance_request_type)
        instance.limit_currency = validated_data.get(
            'limit_currency', instance.limit_currency)
        instance.total_limit_amount = validated_data.get(
            'total_limit_amount', instance.total_limit_amount)
        instance.finance_currency = validated_data.get(
            'finance_currency', instance.finance_currency)
        instance.settlement_currency = validated_data.get(
            'settlement_currency', instance.settlement_currency)
        instance.max_finance_percentage = validated_data.get(
            'max_finance_percentage', instance.max_finance_percentage)
        instance.max_invoice_age_for_funding = validated_data.get(
            'max_invoice_age_for_funding', instance.max_invoice_age_for_funding)
        instance.max_age_for_repayment = validated_data.get(
            'max_age_for_repayment', instance.max_age_for_repayment)
        instance.minimum_period = validated_data.get(
            'minimum_period', instance.minimum_period)
        instance.maximum_period = validated_data.get(
            'maximum_period', instance.maximum_period)
        instance.maximum_amount = validated_data.get(
            'maximum_amount', instance.maximum_amount)
        instance.minimum_amount = validated_data.get(
            'minimum_amount', instance.minimum_amount)
        instance.financed_amount = validated_data.get(
            'financed_amount', instance.financed_amount)
        instance.balance_amount = validated_data.get(
            'balance_amount', instance.balance_amount)
        instance.grace_period = validated_data.get(
            'grace_period', instance.grace_period)
        instance.interest_type = validated_data.get(
            'interest_type', instance.interest_type)
        instance.interest_rate_type = validated_data.get(
            'interest_rate_type', instance.interest_rate_type)
...
class InvoiceCreateserializer(serializers.Serializer):
    ...
    party = serializers.PrimaryKeyRelatedField(queryset=Parties.objects.all())
    pairing = serializers.PrimaryKeyRelatedField(
        queryset=Pairings.objects.all())
    invoice_number = serializers.CharField()
    issue_date = serializers.DateField(format="%d-%m-%Y")
    due_date = serializers.DateField(format="%d-%m-%Y")
    invoice_currency = serializers.PrimaryKeyRelatedField(
        queryset=Currencies.objects.all())
    amount = serializers.DecimalField(max_digits=8, decimal_places=2)
    funding_request_type = serializers.ChoiceField(
        choices=finance_request_type, default=None)
    finance_currency_type = serializers.PrimaryKeyRelatedField(
        queryset=Currencies.objects.all())
    settlement_currency_type = serializers.PrimaryKeyRelatedField(
        queryset=Currencies.objects.all())
    interest_rate = serializers.DecimalField(max_digits=8, decimal_places=2)
    financed_amount = serializers.DecimalField(max_digits=8, decimal_places=2)
    ...
    def create(self, validated_data):
        ...
        settlement_currency_type = validated_data.pop(
        ...
        to_party = validated_data.pop('to_party')
        event_user = validated_data.pop('event_user')
        invoice = Invoices.objects.create(party=party, pairing=pairing, invoice_no=invoice_number, issue_date=issue_date, due_date=due_date, invoice_currency=invoice_currency, amount=amount, funding_req_type=funding_request_type,
                                          finance_currency_type=finance_currency_type, settlement_currency_type=settlement_currency_type, interest_rate=interest_rate, financed_amount=financed_amount, bank_loan_id=bank_loan)
        invoice.save()
        work = workflowitems.objects.create(
            invoice=invoice, current_from_party=from_party, current_to_party=to_party, event_users=event_user)
        work.save()
        event = workevents.objects.create(
            workitems=work, from_party=from_party, to_party=to_party, type="INVOICE")
        event.save()
        return invoice
...
class InvoiceUploadserializer(serializers.Serializer):
    ...
    due_date = serializers.DateField(format="%d/%m/%Y")
    financing_currency = serializers.PrimaryKeyRelatedField(
        queryset=Currencies.objects.all())
    ...
    def create(self, validated_data):
        ...
        uploads = Invoiceuploads.objects.create(
            program_type=program_type, invoices=invoices, **validated_data)
        work = workflowitems.objects.create(
...
class WorkitemInvoiceUploadserializer(serializers.ModelSerializer):
    workflowevent = WorkeventInvoiceUploadsserializer(
