...
from transaction.Schema.programsSchema import financeType, interestRate, interestType
...
class createPairing(graphene.Mutation):
    ...
    def mutate(self, root, program_type, counterparty_id, finance_request, currency, max_total_limit, expiry_date, max_finance_percentage, max_invoice_age_for_funding,
        ...
        program = Programs.objects.get(id=program_type)
        party = Parties.objects.get(id=counterparty_id)

        _pairing = Pairings.objects.create(
            program_type=program, counterparty_id=party, finance_request=finance_request, currency=currency, max_total_limit=max_total_limit, expiry_date=expiry_date, max_finance_percentage=max_finance_percentage, max_invoice_age_for_funding=max_invoice_age_for_funding, max_age_for_repayment=max_age_for_repayment,
...
class updatePairing(graphene.Mutation):
    ...
    def mutate(self, root, id, program_type, counterparty_id, finance_request, currency, max_total_limit, expiry_date, max_finance_percentage, max_invoice_age_for_funding,
               max_age_for_repayment, minimum_period, maximum_period, minimum_amount_currency, minimum_amount, maximum_amount, financed_amount, balance_amount, grace_period, interest_type, interest_rate_type, interest_rate, margin):
        ...
        _pairing.counterparty_id = party
        _pairing.finance_request = finance_request
        _pairing.currency = currency
        _pairing.max_total_limit = max_total_limit
