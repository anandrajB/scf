...
from accounts.models import Countries, Currencies, Parties, User
from transaction.Schema.choices import PartyType
...
class PartiesType_create(graphene.Mutation):
    class Arguments:
        ...
        party_type = graphene.Argument(PartyType)

    parties = graphene.Field(PartiesType)

    def mutate(self, root, email, phone, password, customer_id, name, base_currency, address_line_1, address_line_2, city, state, zipcode, country_code, onboarded, party_type):
        # party_type = Parties.objects.get(id=party_type_id)
...
class PartiesType_update(graphene.Mutation):
    ...
    def mutate(self, root, customer_id, name, base_currency, address_line_1, address_line_2, city, state, zipcode, country_code, onboarded, party_type):
