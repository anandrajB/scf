...
class Ismaker(permissions.BasePermission):
    def has_permission(self, request, view):
        qs = accounts.models.userprocessauth.objects.get(user = request.user ,model = 'INVOICE',  action__desc__contains = 'SUBMIT')
...
class Is_Rejecter(permissions.BasePermission):
    def has_permission(self, request, view):
        qs = accounts.models.userprocessauth.objects.get(user = request.user , model = 'INVOICE', action__desc__contains = "REJECT")
        if qs.data_entry == True:
            return True
...
class Is_Accepter(permissions.BasePermission):
    def has_permission(self, request, view):
        qs = accounts.models.userprocessauth.objects.get(user = request.user , model = 'INVOICE', action__desc__contains = "ACCEPT")
        if qs.data_entry == True:
            return True
