...
class Ismaker(permissions.BasePermission):
    def has_permission(self, request, view):
        qs = accounts.models.userprocessauth.objects.get(
            user=request.user, model='PROGRAM',  action__desc__contains='SUBMIT')
...
class Is_Rejecter(permissions.BasePermission):
    def has_permission(self, request, view):
        user = request.user
        qs = accounts.models.userprocessauth.objects.get(
            user=request.user, model='PROGRAM', action__desc__contains="REJECT")
...
class Is_Accepter(permissions.BasePermission):
    def has_permission(self, request, view):
        user = request.user
        qs = accounts.models.userprocessauth.objects.get(
            user=request.user, model='PROGRAM', action__desc__contains="ACCEPT")
        if qs.data_entry == True and user.is_administrator == True:
            return True
